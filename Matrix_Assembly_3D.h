...
public:
	...
	void build_matrix(Mesh& Reader, COOMatrix& A, Volume_Matrix_Integral3D& Integration, integrand_function3D f, double tol=1e-16);
	...
	void build_vector(Mesh& Reader, DataVector& a, Volume_Vector_Integral3D& Integration, integrand_function3D f);
	...
	void neumann_BC(Mesh& Reader, DataVector& f_vector, Boundary_Vector_Integral3D& Integral, \
		boundary_integrand3D f, on_boundary on_bound, std::vector<double>& params);
	...
	void robin_BC(Mesh& Reader, DataVector& f_vector, COOMatrix& G_matrix, \
		Boundary_Vector_Integral3D& Vect_integral, Boundary_Matrix_Integral3D& Mat_integral, \
		boundary_integrand3D f_vect, boundary_integrand3D f_mat, on_boundary on_bound, std::vector<double>& params, double tol = 1e-16);
