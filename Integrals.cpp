...
double Boundary_Vector_Integral::Evaluate_Integrand(double coord_master, std::vector<std::vector<double>>& coord_deformed, int ind_i, Shape_fct_1D& Shape, double(&f)(double, double))
{
	return f(Shape.Coordinates_deformed(coord_master, coord_deformed, 0), Shape.Coordinates_deformed(coord_master, coord_deformed, 1))\
...
double Boundary_Vector_Integral::Gaussian_Quadrature(int ind_i, std::vector<std::vector<double>>& coord_deformed, Shape_fct_1D& Shape, double(&f)(double, double))
{
...
double Boundary_Matrix_Integral::Evaluate_Integrand(double coord_master, std::vector<std::vector<double>>& coord_deformed, int ind_i, int ind_j, Shape_fct_1D& Shape, double(&f)(double, double))
{
	return f(Shape.Coordinates_deformed(coord_master, coord_deformed, 0), Shape.Coordinates_deformed(coord_master, coord_deformed, 1))\
		* Shape.Evaluate(coord_master, ind_i) * Shape.Evaluate(coord_master, ind_j)\
		* sqrt(pow(coord_deformed[1][0] - coord_deformed[0][0], 2) + pow(coord_deformed[1][1] - coord_deformed[0][1], 2)) / 2;
...
double Boundary_Matrix_Integral::Gaussian_Quadrature(int ind_i, int ind_j, std::vector<std::vector<double>>& coord_deformed, Shape_fct_1D& Shape, double(&f)(double, double))
{
